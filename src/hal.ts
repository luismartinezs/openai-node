...
const hal = async (): Promise<void> => {
  console.log(
    `${BOT_NAME}: Welcome to ${BOT_NAME}! A chatbot with limited memory. Ask anything! ctrl/cmd + C to quit.\n`
  );
  const initialPrompt = (await readFile("./prompts/hal.txt"))?.replace(
    "<<BLOCK>>",
    ""
  );
  if (initialPrompt) {
...
const handleConversation = async (): Promise<void> => {
  ...
  let prompt = (await readFile("./prompts/hal.txt"))?.replace(
    "<<BLOCK>>",
    textBlock
  );
  prompt += `\n${BOT_NAME}:`;
